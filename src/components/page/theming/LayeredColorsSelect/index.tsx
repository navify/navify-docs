...
export default function LayeredColorsSelect({ ...props }) {
  ...
  useEffect(() => {
    setVariations([
      {
        property: `--nav-color-${color}`,
        name: 'Base',
        description: 'The main color that all variations are derived from',
        value: getComputedStyle(el.current).getPropertyValue(`--nav-color-${color}`),
      },
      {
        property: `--nav-color-${color}-rgb`,
        name: 'Base (rgb)',
        rgb: true,
        description: 'The base color in red, green, blue format',
        value: getComputedStyle(el.current).getPropertyValue(`--nav-color-${color}-rgb`),
      },
      {
        property: `--nav-color-${color}-contrast`,
        name: 'Contrast',
        description: 'The opposite of the base color, should be visible against the base color',
        value: getComputedStyle(el.current).getPropertyValue(`--nav-color-${color}-contrast`),
      },
      {
        property: `--nav-color-${color}-contrast-rgb`,
        name: 'Contrast (rgb)',
        rgb: true,
        description: 'The contrast color in red, green, blue format',
        value: getComputedStyle(el.current).getPropertyValue(`--nav-color-${color}-contrast-rgb`),
      },
      {
        property: `--nav-color-${color}-shade`,
        name: 'Shade',
        description: 'A slightly darker version of the base color',
        value: getComputedStyle(el.current).getPropertyValue(`--nav-color-${color}-shade`),
      },
      {
        property: `--nav-color-${color}-tint`,
        name: 'Tint',
        description: 'A slightly lighter version of the base color',
        value: getComputedStyle(el.current).getPropertyValue(`--nav-color-${color}-tint`),
      },
    ]);
  }, [color]);
  ...
  return (
    ...
    >
      <div className={styles.selectRow}>
        <ColorDot color={`var(--nav-color-${color})`} />
        <InputWrapper>
