...
import { Component, Element, Listen, State, h } from '@rindo/core';
    ...
    <div className={clsx(props.className, 'stepped-color-generator')}>
      ...
      <pre className={clsx(styles.codePre)}>
        <code>
          :root {'{'}
          {'\n'}
          {'\t'}--nav-background-color: <CodeColor color={backgroundColor}>{backgroundColor}</CodeColor>;{'\n'}
          {'\t'}--nav-background-color-rgb:{' '}
          <CodeColor color={backgroundColor}>{new Color(backgroundColor).toList()}</CodeColor>;{'\n'}
          {'\n'}
          {'\t'}--nav-text-color: <CodeColor color={textColor}>{textColor}</CodeColor>;{'\n'}
          {'\t'}--nav-text-color-rgb: <CodeColor color={textColor}>{new Color(textColor).toList()}</CodeColor>;{'\n'}
          {'\n'}
          {steppedColors.map((color, i) => (
            <>
              {'\t'}--nav-color-step-{(i + 1) * 50}: <CodeColor color={color}>{color}</CodeColor>;{'\n'}
            </>
          ))}
          {'}'}
          {'\n'}
