...
const DocDemo = (props) => {
  const [navifyMode, setNavifyMode] = useState('ios');
  ...
  const modeToggle = (
    <div className="docs-demo-mode-toggle">
      {['ios', 'md'].map((mode) => (
        <button
          className={clsx({
            'is-selected': mode === navifyMode,
          })}
          title={`Toggle ${mode === 'ios' ? 'iOS' : 'Android'} mode`}
          onClick={() => setNavifyMode(mode)}
        >
  ...
  const sourceLink = (
    <a href={props.source} className="docs-demo-source" target="_blank" title="Demo Source">
      {/* <nav-icon name="open" /> */} View Source
    </a>
  );

  const device = (
    <div className={`docs-demo-device ${navifyMode}`}>
      <figure>
        ...
        <iframe
          ...
          importance="low"
          onLoad={onIframeLoad}
          src={`${props.url}?navify:mode=${navifyMode}`}
          ref={iframe}
        />
      </figure>
    </div>
