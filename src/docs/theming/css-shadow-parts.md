...
:::warning Note
Navify Framework components are **not all** Shadow DOM components. If the component is a Shadow DOM component, there will be a badge in the top right of its `component documentation`. An example of a Shadow DOM component is the `button component`.
:::
...
All exposed parts for an Navify Framework component can be found under the CSS Shadow Parts heading on its API page. To view all components and their API pages, see the `Component documentation`.

In order to have parts a component must meet the following criteria:

- It is a [Shadow DOM](../reference/glossary.md#shadow) component. If it is a [Scoped](../reference/glossary.md#scoped)> or Light DOM component, the child elements can be targeted directly. If a component is Scoped or Shadow, it will be listed by its name on its `component documentation page`.
