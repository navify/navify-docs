...
Navify uses **modes** to customize the look of components. Each **platform** has a default **mode**, but this can be overridden through the global `config`. The following chart displays the default **mode** that is added to each **platform**:
...
There are many global variables that can be overridden by mode, including [Navify's color variables](colors.md), `theme variables` and `global component variables`.
