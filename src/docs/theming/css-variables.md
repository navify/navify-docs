...
Navify provides variables that exist at the component level, such as `--background` and `--color`. For a list of the custom properties a component accepts, view the `CSS Custom Properties` section of its `API reference`. For example, see the `Button CSS Custom Properties`.

### Global Variables

There are several global variables that Navify provides in order to make theming an entire application easier. For more information, see [Colors](colors.md), `Themes` and `Advanced Theming`.
