...
It is recommended to check whether or not the user is on the root page prior to exiting the application. Developers can use the `canGoBack` method on `NavRouter` in Navify Kdu.
...
| Navigation | 0        | Yes        | Applies to routing navigation (i.e. Kdu Routing).                                                                                    |
