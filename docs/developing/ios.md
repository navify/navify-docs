...
<head>
  ...
  <meta
    name="description"
    content="Navify's iOS development guide covers how to build and run Navify apps on iOS simulators and devices. iOS apps can only be developed on macOS with Xcode setup."
  />
</head>

This guide covers how to run and debug Navify apps on iOS simulators and devices using [Jigra](../reference/glossary.md#jigra) or [Cordova](../reference/glossary.md#cordova). iOS apps can only be developed on macOS with Xcode installed.

There are two workflows for running Navify apps on iOS:

- [Running with Xcode](#running-with-xcode)
- [Running with the Navify CLI](#running-with-the-navify-cli)

The Xcode approach is generally more stable, but the Navify CLI approach offers [live-reload](../reference/glossary.md#livereload) functionality.
...
1. **Generate the native project, if it does not already exist.**
   ...
   ```shell
   $ navify jigra add ios
   ```

   For Cordova, run the following:

   ```shell
   $ navify cordova prepare ios
   ```
...
1. **Open the project in <b>Xcode</b>.**
   ...
   For Jigra, run the following to open the app in Xcode:

   ```shell
   $ navify jigra open ios
   ```
...
1. **Develop the Navify app and sync it to the native project.**
   ...
   With each meaningful change, Navify apps must be built into web assets before the change can appear on iOS simulators and devices. The web assets then must be copied into the native project. Luckily, this process is made easy with a single Navify CLI command.

   For Jigra, run the following:

   ```shell
   $ navify jigra copy ios
   $ navify jigra update
   ```
...
## Running with the Navify CLI

The Navify CLI can build, copy, and deploy Navify apps to iOS simulators and devices with a single command. It can also spin up a development server, like the one used in `navify serve`, to provide [live-reload](../reference/glossary.md#livereload) functionality.
...
### Live-reload with Jigra

Jigra does not yet have a way to build native projects. It relies on Xcode to build and deploy app binaries. However, the Navify CLI can boot up a live-reload server and configure Jigra to use it with a single command.
...
Safari has Web Inspector support for iOS simulators and devices. Open the **Develop** menu and select the simulator or device, then select the Navify App to open Web Inspector.
