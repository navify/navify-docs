...
<head>
  ...
  <meta
    name="description"
    content="If errors occur, npm may need to be set up to operate without elevated permissions. Read our Navify app Development Tips for more on resolving permission issues."
  />
</head>
...
For example, to update the `@navify/kdu` package to the release tagged `latest`, run:

```shell
npm install @navify/kdu@latest --save
```
...
By default, when an app is viewed in the browser, Navify will apply the `md` mode. However, since Navify components adapt according to their platform, it is helpful to be able to view what the app will look like on iOS. To do this, add `?navify:mode=ios` to the URL where the app is being served. For example, if the app is served on port `8100`, the url would be: `http://localhost:8100/?navify:mode=ios`.
...
The [Navify CLI](../cli.md) can then be used to run the app in the current directory on the simulator:

```shell
navify cordova emulate ios -lc
```
