...
<head>
  <title>nav-select: Select One or Multiple Value Boxes or Placeholders</title>
  <meta name="description" content="nav-select is represented by selected value(s), or a placeholder, and dropdown icon. When you tap select, a dialog box appears with an easy to select list." />
</head>
...
A select should be used with child `<nav-select-option>` elements. If the child option is not given a `value` attribute then its text will be used as the value.

If `value` is set on the `<nav-select>`, the selected option will be chosen based on that value.
...
By default, select uses [nav-alert](alert.md) to open up the overlay of options in an alert. The interface can be changed to use [nav-action-sheet](action-sheet.md) or [nav-popover](popover.md) by passing `action-sheet` or `popover`, respectively, to the `interface` property. Read on to the other sections for the limitations of the different interfaces.
...
The main ways of handling user interaction with the select are the `navChange`, `navDismiss`, and `navCancel` events. See [Events](#events) for more details on these and other events that select fires.
...
See the [nav-alert docs](alert.md), [nav-action-sheet docs](action-sheet.md), and [nav-popover docs](popover.md) for the properties that each interface accepts.
...
There are two units that make up the Select component and each need to be styled separately. The `nav-select` element is represented on the view by the selected value(s), or placeholder if there is none, and dropdown icon. The interface, which is defined in the [Interfaces](#interfaces) section above, is the dialog that opens when clicking on the `nav-select`. The interface contains all of the options defined by adding `nav-select-option` elements. The following sections will go over the differences between styling these.

### Styling Select Element

As mentioned, the `nav-select` element consists only of the value(s), or placeholder, and icon that is displayed on the view. To customize this, style using a combination of CSS and any of the [CSS custom properties](#css-custom-properties).
