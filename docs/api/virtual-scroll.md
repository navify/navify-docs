...
<head>
  <title>nav-virtual-scroll | Virtual Scroll List for Navify Apps</title>
  <meta name="description" content="nav-virtual-scroll, displays a virtual, infinite list. Records are passed to the virtual scroll containing the data to create templates." />
</head>
...
For virtual scrolling options in Navify React, please see [React Virtual Scroll Guide](../react/virtual-scroll.md).

## Kdu

For virtual scrolling options in Navify Kdu, please see [Kdu Virtual Scroll Guide](../kdu/virtual-scroll.md).

------

The following documentation applies to the `nav-virtual-scroll` component.
...
It's also important to know that Navify's default item sizes have
slightly different heights between platforms, which is perfectly fine.

## Images Within Virtual Scroll

HTTP requests, image decoding, and image rendering can cause jank while
scrolling. In order to better control images, Navify provides `<nav-img>`
to manage HTTP requests and image rendering. While scrolling through items
quickly, `<nav-img>` knows when and when not to make requests, when and
when not to render images, and only loads the images that are viewable
after scrolling. [Read more about `nav-img`.](img.md)
...
features. We recommend using the `<nav-img>` component over the native
...
`<nav-img>` is governed by the containing `nav-content` and does not render
...
WKWebView plugin in order to take advantage of iOS's higher performing
webview. Additionally, WKWebView is superior at scrolling efficiently
in comparison to the older UIWebView.
...
### Use `nav-img` for images

When including images within Virtual Scroll, be sure to use
[`nav-img`](img.md) rather than the standard `<img>` HTML element.
With `nav-img`, images are lazy loaded so only the viewable ones are
...
different identities, and Navify will tear down the entire DOM and rebuild
...
```html
<nav-content>
  <nav-virtual-scroll [items]="items" approxItemHeight="320px">
    <nav-card *virtualItem="let item; let itemBounds = bounds;">
      <div>
        <nav-img [src]="item.imgSrc" [height]="item.imgHeight" [alt]="item.name"></nav-img>
      </div>
    <nav-card-header>
      <nav-card-title>{{ item.name }}</nav-card-title>
    </nav-card-header>
    <nav-card-content>{{ item.content }}</nav-card-content>
    </nav-card>
  </nav-virtual-scroll>
</nav-content>
```
...
The array of records should be passed to the `items` property on the `nav-virtual-scroll` element.
The data given to the `items` property must be an array. An item template with the `*virtualItem` property is required in the `nav-virtual-scroll`. The `*virtualItem` property can be added to any element.

```html
<nav-virtual-scroll [items]="items">
  <nav-item *virtualItem="let item">
    {{ item }}
  </nav-item>
</nav-virtual-scroll>
```
...
```html
<nav-virtual-scroll [items]="items" [headerFn]="myHeaderFn">
  <nav-item-divider *virtualHeader="let header">
    {{ header }}
  </nav-item-divider>
  <nav-item *virtualItem="let item">
    Item: {{ item }}
...
between the 39th and 40th, and so on, a `<nav-item-divider>` will
