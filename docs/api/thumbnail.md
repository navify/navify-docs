...
<head>
  <title>nav-thumbnail | Thumbnail App Component to Wrap Images or Icons</title>
...
Thumbnails can be used by themselves or inside of any element. If placed inside of an `nav-item`, the thumbnail will resize to fit the parent component. To position a thumbnail on the left or right side of an item, set the slot to `start` or `end`, respectively.
