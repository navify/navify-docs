...
<head>
  ...
  <meta name="description" content="Nav-toolbar component lets you customize toolbar buttons on your app menu. Add fixed toolbars above or below content or use full screen to scroll with content." />
</head>
...
In `md` mode, the `<nav-header>` will receive a box-shadow on the bottom, and the `<nav-footer>` will receive a box-shadow on the top.  In `ios` mode, the `<nav-header>` will receive a border on the bottom, and the `<nav-footer>` will receive a border on the top.
