...
Clicking the collapsed indicator will fire the `navCollapsedClick` event. This can be used to, for example, expand the breadcrumbs.
...
The `navCollapsedClick` event can also be used to present an overlay (in this case, an `nav-popover`) showing the hidden breadcrumbs.
