...
<head>
  <title>nav-avatar: Circular Application Avatar Icon Component</title>
  <meta name="description" content="Nav-avatars are circular application components that wrap an image or icon. They can represent a person or an object, by themselves or inside of any element." />
</head>
...
Avatars can be used by themselves or inside of any element. If placed inside of an `nav-chip` or `nav-item`, the avatar will resize to fit the parent component. To position an avatar on the left or right side of an item, set the slot to `start` or `end`, respectively.
