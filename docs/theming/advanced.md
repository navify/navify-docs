...
<head>
  <title>Advanced Theming: Quickly Customize App Colors using CSS | Navify</title>
  <meta
    name="description"
    content="CSS-based theming enables apps to customize colors quickly by loading a CSS file or changing CSS property values. Read to learn about Navify Advanced Theming."
  />
</head>
...
:::note
The `theme-color` meta controls the interface theme when running in a web browser or as a PWA and has no effect when an app is deployed using Jigra or Cordova. If you are looking to customize the area under the status bar, we recommend using the [Jigra Status Bar Plugin](https://jigrajs.web.app/docs/apis/status-bar).
:::
...
| Name                      | Description                                                                                |
| ------------------------- | ------------------------------------------------------------------------------------------ |
| `--nav-font-family`       | Font family of the app                                                                     |
| `--nav-statusbar-padding` | Statusbar padding top of the app                                                           |
| `--nav-safe-area-top`     | Adjust the safe area inset top of the app                                                  |
| `--nav-safe-area-right`   | Adjust the safe area inset right of the app                                                |
| `--nav-safe-area-bottom`  | Adjust the safe area inset bottom of the app                                               |
| `--nav-safe-area-left`    | Adjust the safe area inset left of the app                                                 |
| `--nav-margin`            | Adjust the margin of the [Margin attributes](../layout/css-utilities.md#element-margin)    |
| `--nav-padding`           | Adjust the padding of the [Padding attributes](../layout/css-utilities.md#element-padding) |

### Grid Variables

| Name                           | Description                                    |
| ------------------------------ | ---------------------------------------------- |
| `--nav-grid-columns`           | Number of columns in the grid                  |
| `--nav-grid-padding-xs`        | Padding of the grid for xs breakpoints         |
| `--nav-grid-padding-sm`        | Padding of the grid for sm breakpoints         |
| `--nav-grid-padding-md`        | Padding of the grid for md breakpoints         |
| `--nav-grid-padding-lg`        | Padding of the grid for lg breakpoints         |
| `--nav-grid-padding-xl`        | Padding of the grid for xl breakpoints         |
| `--nav-grid-column-padding-xs` | Padding of the grid columns for xs breakpoints |
| `--nav-grid-column-padding-sm` | Padding of the grid columns for sm breakpoints |
| `--nav-grid-column-padding-md` | Padding of the grid columns for md breakpoints |
| `--nav-grid-column-padding-lg` | Padding of the grid columns for lg breakpoints |
| `--nav-grid-column-padding-xl` | Padding of the grid columns for xl breakpoints |
...
Navify uses colors with an opacity (alpha) in several components. In order for this to work, those properties must be provided in RGB format. When changing any of the properties that have a variation ending in `-rgb`, it is important they are also provided in a comma separated format **without parentheses**. Below are some examples for changing text and background color.

```css
:root {
  /* These examples use the same color: sienna. */
  --nav-text-color: #a0522d;
  --nav-text-color-rgb: 160, 82, 45;

  /* These examples use the same color: lightsteelblue. */
  --nav-background-color: #b0c4de;
  --nav-background-color-rgb: 176, 196, 222;
}
```

Note that the RGB formatted colors are the exact same color as the hex properties, but can now be used with `rgba()`. For example, `--nav-text-color-rgb` can now be used in the following way

```css
body {
  color: rgba(var(--nav-text-color-rgb), 0.25);
}
```
...
This is normally not a problem, but when an application needs to have dynamic theming it presents issues. In Navify, this is the reason that there are [variations to each color](colors.md#layered-colors), and it is also why [stepped colors](themes.md#stepped-colors) are necessary for theming.
