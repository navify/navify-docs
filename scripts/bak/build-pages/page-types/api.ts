import { components } from '@navify/docs/core.json';
...
const getAPIPages = async (): Promise<Page[]> => {
  const pages = components.map(async component => {
    ...
    const meta = {
      title: `${component.tag}: Navify Framework API Docs`,
      description: `The ${component.tag} component is one of many Navify Framework components used to build apps for Android, iOS, and Progressive Web Apps`
    };
...
const getDemoUrl = async (component: any) => {
  ...
  if (hasDemo) {
    return {
      ...
      demoSourceUrl: `https://github.com/navify/navify-docs/tree/main/src/demos/${demoPath}`
    };
  }
};
